...
Public Class Main
    ...
    Public Function IsSupported(ByVal Filename As String) As Boolean Implements iFileFormat.IsSupported
        ...
        FileOpen(L, Filename, OpenMode.Input)
        Do
            Text = LineInput(L)
    ...
    Sub Load(ByVal Filename As String) Implements iFileFormat.Load
        ...
        Catch ex As Exception
            RaiseEvent WasError(iFileFormat.scdError.scdCantOpen)
            Exit Sub
        End Try
        Mx = FileLen(Filename)
        Do
    ...
    Public Sub Save(ByVal FileName As String) Implements iFileFormat.Save
        ...
        Dim L As Integer = FreeFile(), O As Integer = 0
        Dim I As Integer = 0, K As Integer = UBound(Me.Items)
...
start:
    ...
    Public Sub Import()
        ...
        For Each item As Database.Item In Me.db.Select(sql, Me.application_guid)
            I = item.ID
            If item.Type.ToLower = "scdfile" Then
